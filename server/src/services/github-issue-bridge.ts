...
export function parseGitHubRemoteUrl(remoteUrl: string | null | undefined): string | null {
...
export function githubIssueBridge(db: Db, deps: GithubIssueBridgeDependencies = {}) {
  ...
  return {
    async syncProject(
      ...
      // Deduplicate using originKind=github_issue + originId=${repo}#${number}
      const existingIssues = await db
        ...
        .where(
          and(
            eq(issuesTable.companyId, project.companyId),
            eq(issuesTable.projectId, project.id),
            eq(issuesTable.originKind, "github_issue"),
          ),
        );
      const existingOriginIds = new Set<string>(
        (existingIssues as Array<{ originKind: string | null; originId: string | null }>)
      ...
      for (const ghIssue of ghIssues) {
        ...
        const originId = `${ghRepo}#${ghIssue.number}`;
        ...
        existingOriginIds.add(originId);
      }
