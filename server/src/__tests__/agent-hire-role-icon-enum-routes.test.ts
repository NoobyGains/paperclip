/**
 * Regression test for #55 — widen agent role enum (+reviewer) and icon enum (+shield-check).
 *
 * Verifies that POST /api/companies/:id/agent-hires accepts role="reviewer"
 * and icon="shield-check" without a 400 validation error now that both values
 * have been added to the shared AGENT_ROLES / AGENT_ICON_NAMES enums.
 */
...
describe("role and icon enum widening (#55)", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("accepts role=reviewer on POST /api/companies/:id/agent-hires", async () => {
  ...
  it("accepts icon=shield-check on POST /api/companies/:id/agent-hires", async () => {
    const app = await createApp(adminActor);
    ...
    expect(mockAgentService.create).toHaveBeenCalledWith(
      companyId,
      expect.objectContaining({ icon: "shield-check" }),
    );
  });

  it("rejects an unknown role value on POST /api/companies/:id/agent-hires", async () => {
    const app = await createApp(adminActor);
