...
If you are the CEO and your company has zero issues, run the first-contact flow from your onboarding instructions before normal issue picking: orient on the repo, import GitHub issues, hire specialists without sending `adapterType` when a default hire adapter exists, and let Paperclip auto-attach reviewers when auto-review is enabled.
